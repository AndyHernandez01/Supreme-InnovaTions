...
""" 
Combines the bluetooth client, flow meter code, and LEDs
"""
...
def get_flow_rate() -> float:  # Tallies the counts in an interval and calculates flow rate
    ...
    final_count = count  # Store the final count

    if final_count == 0:  # Turn the LEDs red when there is no flow
        LED.led_red()
    else:
        LED.led_blue()  # Turn the LEDs blue if there is flow
    return round(final_count / (7.5 * FM_INTERVAL), 2)  # Return flow rate
...
LED.short_pulse()  # Pulse blue 3 times to show code is booting up
LED.short_pulse()
LED.short_pulse()
